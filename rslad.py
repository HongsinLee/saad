...
for epoch in range(1, args.epochs + 1):
    ...
    for step, (X, y) in enumerate(trainloader):
        ...
        inputs_adv = rslad_inner_loss(student, teacher_logits, X,step_size=8/255.0, epsilon=2/255.0, perturb_steps=10)
        optimizer.zero_grad()
        student_plus = student(inputs_adv)
        student_logit = student(X)

        kl_Loss1 = criterion_kl(F.log_softmax(student_plus, dim=1), F.softmax(teacher_logits, dim=1))
        kl_Loss2 = criterion_kl(F.log_softmax(student_logit, dim=1), F.softmax(teacher_logits, dim=1))
